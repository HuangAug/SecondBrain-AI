...
class ConversationActions {
  ...
  Future<void> deleteConversations(List<String> conversationIds) async {
    await _api.dio.post('/conversations/bulk-delete', data: {
      'ids': conversationIds,
    });
  }
}
...
final conversationActionsProvider = Provider<ConversationActions>((ref) {
  return ConversationActions(ref.watch(apiClientProvider));
});
...
class ChatStreamNotifier extends StateNotifier<List<ChatMessage>> {
  ...
  Future<void> sendMessage(String text, {String? conversationId}) async {
    ...
    try {
        ...
        for (final line in lines) {
          ...
          if (data['content'] != null &&
            ...
            final updated = state.last
                .copyWith(content: buffer.toString(), isStreaming: true);
          ...
          if (data['done'] == true) {
            ...
            final finalMsg =
                state.last.copyWith(isStreaming: false, citations: citations);
      ...
      final errorMsg =
          state.last.copyWith(content: '发送失败：$e', isStreaming: false);
...
final chatStreamProvider =
    StateNotifierProvider<ChatStreamNotifier, List<ChatMessage>>((ref) {
